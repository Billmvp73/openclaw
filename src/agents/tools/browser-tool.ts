...
export function createBrowserTool(opts?: {
  ...
  const effectiveProductName = opts?.productName ?? "OpenClaw";
  return {
    label: "Browser",
    name: "browser",
    description: [
      `Control the browser via ${effectiveProductName}'s browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).`,
      ...
      `Chrome extension relay needs an attached tab: user must click the ${effectiveProductName} Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it.`,
